...
\subsection{Heurística Greedy con Mejora 2-Opt}

\subsubsection{Descripción del Algoritmo}

La heurística greedy utiliza el algoritmo del **vecino más cercano** (Nearest Neighbor) para construir una solución rápida, seguida de una mejora local con **2-Opt**.

\textbf{Fase 1 - Construcción Greedy}:
\begin{enumerate}
    \item Comenzar en ciudad 0
    \item Repetir: Ir a la ciudad no visitada más cercana
    \item Retornar a ciudad inicial
\end{enumerate}

\textbf{Fase 2 - Mejora 2-Opt}:
\begin{enumerate}
    \item Para cada par de aristas en el tour
    \item Si intercambiarlas reduce la distancia, hacerlo
    \item Repetir hasta que no haya mejoras
\end{enumerate}

\subsubsection{Pseudocódigo}

\begin{algorithm}[H]
\caption{Greedy Nearest Neighbor}
\begin{algorithmic}[1]
\Procedure{GreedyNearestNeighbor}{$distance\_matrix$}
    \State $current \gets 0$ \Comment{Ciudad inicial}
    \State $visited \gets \{0\}$
    \State $tour \gets [0]$
    \State $total\_distance \gets 0$
    \While{$|visited| < n$}
        \State $nearest \gets null$
        \State $min\_dist \gets \infty$
        \For{cada ciudad $next$ no visitada}
            \If{$distance[current][next] < min\_dist$}
                \State $min\_dist \gets distance[current][next]$
                \State $nearest \gets next$
            \EndIf
        \EndFor
        \State $visited \gets visited \cup \{nearest\}$
        \State $tour.append(nearest)$
        \State $total\_distance \gets total\_distance + min\_dist$
        \State $current \gets nearest$
    \EndWhile
    \State $total\_distance \gets total\_distance + distance[current][0]$ \Comment{Retorno}
    \State \Return $(tour, total\_distance)$
\EndProcedure
\end{algorithmic}
\end{algorithm}

\begin{algorithm}[H]
\caption{Mejora 2-Opt}
\begin{algorithmic}[1]
\Procedure{TwoOptImprovement}{$tour, distance\_matrix$}
    \State $improved \gets true$
    \While{$improved$}
        \State $improved \gets false$
        \For{$i \gets 0$ to $n-2$}
            \For{$j \gets i+2$ to $n-1$}
                \State $current\_cost \gets dist[tour[i]][tour[i+1]] + dist[tour[j]][tour[j+1]]$
                \State $new\_cost \gets dist[tour[i]][tour[j]] + dist[tour[i+1]][tour[j+1]]$
                \If{$new\_cost < current\_cost$}
                    \State Invertir segmento $tour[i+1:j+1]$
                    \State $improved \gets true$
                \EndIf
            \EndFor
        \EndFor
...
\textbf{Fase Greedy}: $O(n^2)$
\begin{itemize}
    \item Para cada una de las $n$ ciudades
    \item Buscar la más cercana entre las no visitadas: $O(n)$
    \item Total: $O(n \cdot n) = O(n^2)$
\end{itemize}

\textbf{Fase 2-Opt}: $O(n^2 \cdot k)$ donde $k$ es el número de iteraciones
\begin{itemize}
    \item Cada iteración examina $O(n^2)$ pares de aristas
    \item En práctica, $k$ es pequeño (típicamente $k < 100$)
    \item Peor caso: $O(n^3)$ si $k = O(n)$
\end{itemize}

\textbf{Total}: $O(n^2)$ dominante para construcción, $O(n^2 \cdot k)$ para mejora

\textbf{Complejidad Espacial}: $O(n)$
\begin{itemize}
    \item Tour: $O(n)$
    \item Conjunto de visitados: $O(n)$
    \item Matriz de distancias: $O(n^2)$ (compartida)
\end{itemize}

\subsubsection{Implementación}

La implementación completa se encuentra en \texttt{backend/app/core/tsp\_greedy.py}.

Características técnicas:
\begin{itemize}
    \item Construcción greedy simple y eficiente
    \item 2-Opt con early stopping (cuando no hay mejoras)
    \item Máximo de iteraciones configurable (default: 1000)
    \item Estadísticas de mejora: longitud inicial vs final
    \item Sin límite de puntos (escalable a 100+ puntos)
\end{itemize}

\subsubsection{Resultados Experimentales}

\textbf{Datos de Prueba}: Red de Chapinero, 20 puntos

\begin{table}[H]
\centering
\caption{Rendimiento de Greedy + 2-Opt}
\begin{tabular}{lrr}
\toprule
\textbf{Métrica} & \textbf{Valor} & \textbf{Unidad} \\
\midrule
Tour inicial (Greedy) & 26.32 & km \\
Tour mejorado (2-Opt) & 23.20 & km \\
Mejora & 11.86 & \% \\
Intercambios 2-Opt & 9 & swaps \\
Tiempo total & $<$ 100 & ms \\
\bottomrule
\end{tabular}
\end{table}
...
\begin{table}[H]
\centering
\caption{Tiempos con diferentes tamaños}
\begin{tabular}{rrr}
\toprule
\textbf{Puntos} & \textbf{Tiempo (ms)} & \textbf{Distancia (km)} \\
\midrule
10 & $<$ 10 & 12.45 \\
20 & $<$ 100 & 23.20 \\
30 & $<$ 200 & 34.67 \\
...
Se implementaron 12 pruebas exhaustivas:

\begin{enumerate}
    \item \texttt{test\_single\_point}: Caso trivial
    \item \texttt{test\_two\_points}: Caso base
    \item \texttt{test\_three\_points\_greedy}: Construcción greedy básica
    \item \texttt{test\_four\_points\_greedy}: Caso pequeño
    \item \texttt{test\_2opt\_improvement}: Verificación de mejora 2-Opt
    \item \texttt{test\_greedy\_with\_2opt\_combined}: Pipeline completo
    \item \texttt{test\_disconnected\_graph\_error}: Detección de grafo no conexo
    \item \texttt{test\_empty\_points\_error}: Manejo de entrada vacía
    \item \texttt{test\_performance\_on\_larger\_set}: Escalabilidad (30 puntos)
    \item \texttt{test\_greedy\_vs\_optimal\_approximation}: Calidad de aproximación
    \item \texttt{test\_tour\_statistics}: Cálculo de estadísticas
    \item \texttt{test\_real\_data\_integration}: Integración con datos OSM
\end{enumerate}

\textbf{Resultado}: \texttt{12 passed in 13.65s}

\subsubsection{Garantías de Aproximación}

\textbf{Greedy Nearest Neighbor}:
\begin{itemize}
    \item Ratio de aproximación teórico: $O(\log n)$
    \item En práctica: típicamente 1.15-1.5× óptimo
    \item Peor caso conocido: 2× óptimo
\end{itemize}

\textbf{Con mejora 2-Opt}:
\begin{itemize}
    \item Mejora empírica: 10-20\% adicional
    \item Sin garantía teórica, pero muy efectivo en práctica
    \item Resultado típico: 1.05-1.25× óptimo
\end{itemize}
...
\textbf{Ventajas}:
\begin{itemize}
    \item \textbf{Muy rápido}: $O(n^2)$ vs exponencial
    \item \textbf{Escalable}: Funciona con 100+ puntos
    \item \textbf{Buena calidad}: 1.05-1.25× óptimo típicamente
    \item \textbf{Simple de implementar}: Algoritmo intuitivo
    \item \textbf{Siempre termina}: Garantía de convergencia
\end{itemize}

\textbf{Limitaciones}:
\begin{itemize}
    \item \textbf{No garantiza óptimo}: Solución aproximada
    \item \textbf{Greedy miope}: Decisiones locales pueden ser subóptimas
    \item \textbf{2-Opt local}: Puede quedar en mínimo local
    \item \textbf{Dependiente del inicio}: Punto inicial afecta resultado
\end{itemize}

\textbf{Uso Recomendado}:
\begin{itemize}
    \item Problemas grandes ($n > 20$) donde exactos son impracticables
    \item Cuando se requiere respuesta rápida
    \item Solución inicial para algoritmos más sofisticados
    \item Aplicaciones en tiempo real
\end{itemize}

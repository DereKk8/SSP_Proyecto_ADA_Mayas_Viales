...
if len(snapped_points) > MAX_BRUTEFORCE_POINTS:
    ...
    warning_message = (
        f"⚠️ Only the first {MAX_BRUTEFORCE_POINTS} out of "
        f"{original_count} points were used for brute-force "
        f"calculation. Points {MAX_BRUTEFORCE_POINTS + 1}-"
        f"{original_count} were ignored. For larger datasets, "
        f"consider using Held-Karp or Heuristic algorithms."
    )
\end{lstlisting}

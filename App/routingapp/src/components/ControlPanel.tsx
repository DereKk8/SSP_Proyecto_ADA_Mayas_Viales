...
import type { NetworkResponse, PointsResponse, TSPResult, AlgorithmType } from '@/types';
...
export default function ControlPanel({
  ...
  const [selectedAlgorithm, setSelectedAlgorithm] = useState<AlgorithmType>('bruteforce');
  ...
  return (
    <div className="h-full w-96 bg-white shadow-lg overflow-y-auto flex flex-col">
      <div className="p-6 flex-1">
        ...
        <div className="space-y-4 mb-6">
          ...
          {networkData && (
            <div className="text-sm text-gray-600 bg-gray-50 p-3 rounded">
              <p className="font-medium text-gray-900 mb-1">Network loaded:</p>
              <p>Nodes: {networkData.stats.nodes.toLocaleString()}</p>
              <p>Edges: {networkData.stats.edges.toLocaleString()}</p>
            </div>
          )}
          ...
          {pointsData && (
            <div className="text-sm text-gray-600 bg-gray-50 p-3 rounded">
              <p className="font-medium text-gray-900 mb-1">Points snapped:</p>
          ...
          <div className="border-t pt-6 mt-6">
            ...
            <div className="space-y-3 mb-4">
              <label className="flex items-center gap-3 p-3 border rounded cursor-pointer hover:bg-gray-50">
                ...
                />
                <div>
                  <p className="font-medium text-gray-900">Brute-Force</p>
                  <p className="text-xs text-gray-500">Exact, n ≤ 10</p>
                </div>
              </label>
              ...
              <label className="flex items-center gap-3 p-3 border rounded cursor-pointer hover:bg-gray-50">
                ...
                />
                <div>
                  <p className="font-medium text-gray-900">Held-Karp DP</p>
                  <p className="text-xs text-gray-500">Exact, n ≤ 20</p>
                </div>
              </label>
              ...
              <label className="flex items-center gap-3 p-3 border rounded cursor-pointer hover:bg-gray-50">
                ...
                />
                <div>
                  <p className="font-medium text-gray-900">2-Opt Heuristic</p>
                  <p className="text-xs text-gray-500">Approximate, scalable</p>
                </div>
              </label>
            </div>
            ...
            <button
              ...
              className="w-full bg-blue-600 text-white py-2 px-4 rounded font-medium
                       ...
                       transition-colors"
            >
              {loading.algorithm ? 'Computing...' : 'Run Algorithm'}
            </button>
        ...
        {/* Results Display */}
        {(results.bruteforce || results.heldkarp || results.heuristic) && (
          <div className="border-t pt-6 mt-6">
            <h2 className="text-lg font-semibold text-gray-900 mb-4">
              Results
            </h2>
            ...
            <div className="space-y-3">
              {results.bruteforce && (
                <div className="bg-red-50 p-3 rounded border-l-4 border-red-500">
                  ...
                  <p className="text-sm text-gray-600">
                    Length: {formatLength(results.bruteforce.length)}
                  </p>
                  <p className="text-sm text-gray-600">
                    Time: {formatRuntime(results.bruteforce.runtime_ms)}
                  </p>
                </div>
              )}
              ...
              {results.heldkarp && (
                <div className="bg-green-50 p-3 rounded border-l-4 border-green-500">
                  <p className="font-medium text-gray-900">Held-Karp</p>
                  <p className="text-sm text-gray-600">
                    Length: {formatLength(results.heldkarp.length)}
                  </p>
                  <p className="text-sm text-gray-600">
                    Time: {formatRuntime(results.heldkarp.runtime_ms)}
                  </p>
                </div>
              )}
              ...
              {results.heuristic && (
                <div className="bg-purple-50 p-3 rounded border-l-4 border-purple-500">
                  <p className="font-medium text-gray-900">2-Opt Heuristic</p>
                  <p className="text-sm text-gray-600">
                    Length: {formatLength(results.heuristic.length)}
                  </p>
                  <p className="text-sm text-gray-600">
                    Time: {formatRuntime(results.heuristic.runtime_ms)}
                  </p>
                </div>
              )}
            </div>
      ...
      <div className="border-t p-4 bg-gray-50">
        <p className="text-xs text-gray-600">
          💡 <span className="font-medium">Tip:</span> The legend on the map shows all visualization elements.
        </p>
      </div>
    </div>
  );
